...
async function fetchOnce(url, { timeout, headers, text, body }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // Which limit was hit decides what to do about it — a minutely cap clears
      // itself in under a minute, a daily one does not — and the upstream says
      // so in the body. Without this the only way to tell them apart is to poll
      // the live site until it recovers, which is how the last one was
      // diagnosed. Bounded and best-effort: a body we cannot read must not turn
      // a rate limit into a different error.
      let reason = '';
      if (res.status === 429) {
        reason = await res
          .text()
          .then((body) => {
            // Open-Meteo names the limit in `reason`. Anything else is only
            // worth repeating if it is short prose rather than a JSON envelope
            // that says nothing beyond what the status code already said.
            const said =
              body.match(/"reason"\s*:\s*"([^"]{0,160})"/)?.[1] ||
              (body.trim().startsWith('{') ? '' : body.trim().slice(0, 160));
            return said ? ` (${said.trim()})` : '';
          })
          .catch(() => '');
      }
      const err = new Error(`upstream ${res.status} for ${host}${reason}`);
...
export async function fetchJSON(url, { timeout = 15000, headers = {}, text = false, retries = 1, body } = {}) {
  let lastError;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      return await fetchOnce(url, { timeout, headers, text, body });
    } catch (err) {
      lastError = err;
...
export function num(value, { min, max, fallback = null } = {}) {
  // A missing query parameter arrives as null, and `Number(null)` is 0 — a
  // perfectly valid latitude, so an absent coordinate used to read as a point
  // in the Gulf of Guinea rather than as nothing at all.
