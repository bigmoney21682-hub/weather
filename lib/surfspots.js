...
export async function surfSpots(lat, lon, { miles = 30, clusterMiles = 0.75 } = {}) {
  return cached(`surfspots:${gridKey(lat, lon)}:${miles}`, WEEK, async () => {
    ...
    if (!shore.ocean.length) return { spots: [], axisBearing: 0 };
    const found = await candidates(shore.ocean);
    if (!found.length) return { spots: [], axisBearing: 0 };
    ...
    // Clipped to the radius asked for. The coastline behind all this is fetched
    // from a much wider box — the rays need land beyond the search area to run
    // into — and without this the list off West Palm Beach runs on across the
    // Straits of Florida and into the Bahamas.
    const within = dedupe(scored, clusterMiles).filter((s) => haversineMiles(lat, lon, s.lat, s.lon) <= miles);
