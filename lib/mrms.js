...
async function inflatePng(png) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < stride; x++) {
      ...
      const b = y > 0 ? raw[above + x] : 0;
      let v = raw[row + x];
      if (filter === 1) v += a;
      else if (filter === 2) v += b;
      else if (filter === 3) v += (a + b) >> 1;
      else if (filter === 4) {
        const c = x >= bpp && y > 0 ? raw[above + x - bpp] : 0;
        const guess = a + b - c;
...
async function drain() {
  ...
  try {
    while (waiting.size) {
      ...
      let field = null;
      try {
        field = await decodeField(await fetchGz(stamp));
      } catch (err) {
...
/** One rendered tile, from cache if it has been drawn before. */
