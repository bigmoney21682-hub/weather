...
async function jtwcStorms() {
  ...
  const storms = [];
  const blockRe = /<b>\s*([^<]*?(?:Typhoon|Tropical Storm|Tropical Depression|Cyclone|Super Typhoon|Hurricane)[^<]*?)\s*Warning\s*#?(\d+)\s*<\/b>[\s\S]*?products\/([a-z]{2}\d{4})web\.txt/gi;
  let m;
  while ((m = blockRe.exec(xml))) {
    const heading = m[1].replace(/\s+/g, ' ').trim();
    const stormId = m[3]; // e.g. wp1226
