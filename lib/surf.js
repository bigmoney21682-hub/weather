...
import { bearingDegrees, cached, cToF, fetchJSON, haversineMiles, offsetMiles, sunTimes } from './util.js';
import { BEACH_RINGS, beachesWithin } from './places.js';
...
export async function surfableBeach(lat, lon) {
  ...
  return cached(`beach:surfable:${lat.toFixed(2)}:${lon.toFixed(2)}`, 24 * 60 * 60 * 1000, async () => {
    const found = await findSurfableBeach(lat, lon);
    // Coming up empty nearly always means a throttled geocoder rather than a
    // coast with no beaches on it, and caching that would blank the spot name
    // for a whole day. Fail instead — `cached` does not store failures, so the
    // next request gets a fresh try.
    if (!found) throw new Error('no named beach resolved nearby');
    return found;
  }).catch(() => null);
}
...
// How close a named beach has to be before we call a swell point "off" it.
const NAMED_BREAK_MILES = 10;
  ...
  spotSeries.forEach((s, i) => {
    ...
    if (!biggest || h > biggest.waveFt) {
      biggest = {
        ...
        bearingDeg: Math.round(bearingDegrees(originLat, originLon, pt.lat, pt.lon)),
      };
    }
  });

  // That winning point sits out in open water, where reverse geocoding either
  // draws a blank or, worse, confidently names a neighbourhood tens of miles
  // inland. Name it off the beaches we already have instead: the same cached
  // Nominatim search that found the spot, widened to the scan radius. A surfer
  // wants the break it lines up with anyway, not the nearest post office.
  if (biggest) {
    const nearby = await beachesWithin(originLat, originLon, radiusMiles, { limit: 40 }).catch(() => []);
    let closest = null;
    for (const b of nearby) {
      const d = haversineMiles(biggest.lat, biggest.lon, b.lat, b.lon);
      if (!closest || d < closest.distanceMiles) closest = { name: b.name, distanceMiles: d };
    }
    // The biggest wave is nearly always at the outer edge of the scan, tens of
    // miles out, where the nearest beach on the list is far too distant to be
    // what it is breaking "off". Claim a beach only when it is genuinely close;
    // otherwise the bearing-and-distance line carries the location on its own.
    if (closest && closest.distanceMiles <= NAMED_BREAK_MILES) {
      closest.distanceMiles = Math.round(closest.distanceMiles * 10) / 10;
      biggest.place = closest;
    }
  }
