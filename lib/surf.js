...
// somewhere you could actually paddle out.
...
async function biggestAtSpots(spots, lat, lon) {
  const reach = spots
    .map((s) => ({ ...s, fromViewer: haversineMiles(lat, lon, s.lat, s.lon) }))
    .filter((s) => s.fromViewer <= BIGGEST_WAVE_MILES)
...
export async function getSurf(lat, lon, { radiusMiles = 30, snapToBeach = true, at = null } = {}) {
  ...
  const biggest = await biggestAtSpots(spots, lat, lon).catch(() => null);
  ...
  return {
    ...
    hourly: series,
    biggest,
    scanRadiusMiles: BIGGEST_WAVE_MILES,
    waterTempF,
    waterTempSource,
    tideStation: station,
    tides: daytime.list,
